class LongTasks
  ...
  def expire_members
    User.where(approved:true, membership_expired: false).where("membership_updated <= ?", Time.current - 35.days).each do |user|
  ...
  def warn_members
    User.where(approved:true, membership_expired: false).where("membership_updated <= ?", Time.current - 30.days).each do |user|
